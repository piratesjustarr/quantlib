...
Load libs and local data from Quantlib.R

Wrap Strat1 in a big function and pass out buy/sell events to a nice dataframe.
Wrap Idiot strat in a big function and pass out buy/sell events to a nice dataframe.

Display the data.

```{r Load Strategy 1, echo=FALSE, message=FALSE, warning=FALSE}
...
BuyPrice<-as.numeric(Cl(MySymbols[[ChosenSymbol]][TheDate]))
n=21
...
MakeTrades<-function(TheDate)
{
Trades<-as.data.frame(IdiotTrader(TheDate))
for(i in 1:4)
{
  Trades<-cbind(Trades,IdiotTrader(TheDate))
}
Trades<-t(Trades)
...
FakeToday="2017-05-20"
print(paste("Checking: ",FakeToday))
# if(is.weekend(FakeToday))
# {
#   print("Weekdays only mate.")
# }
# else
# {
pmargin=1.1
...
#MakeTrades(TheDate = FakeToday)
StratTrades$HoldTime=as.Date(StratTrades$SellDate)-as.Date(StratTrades$BuyDate)
print(StratTrades)
#print(summary(StratTrades))
#print(IdiotTrades)
#print(summary(IdiotTrades))
# }