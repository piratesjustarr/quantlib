...
suppressMessages(getSymbols(symbols, from = from, to = to,  src = "google", adjust = TRUE))
...
chartStrat<-function(name,object)
{
chart.Posn(portfolio.st, name)
TA_CRSI <- connorsRSI(Cl(object), nRSI = nRSI, nStreak = nStreak, nPercentLookBack = nPercentLookBack)
add_TA(TA_CRSI, col = "red")

TA_cumCRSI <- cumCRSI(price = Cl(object),nCum = nCum, nRSI = nRSI, nStreak = nStreak,nPercentLookBack = nPercentLookBack)
add_TA(TA_cumCRSI, col = "blue")

TA_lagATR <- lagATR(HLC = HLC(object), n = period)
add_TA(TA_lagATR, col = "purple")

TA_SMA <- SMA(Cl(object), n = nSMA)
...
initPortf(portfolio.st, symbols = symbols,
  initDate = initDate, currency = 'GBP')
